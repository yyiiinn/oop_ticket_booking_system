export default {
  ...
  methods: {
    ...
    validateSeatingOptions() {
      if (this.formData.seatingOptions.length === 0) {
        this.formErrors.seatingOptionsGeneral =
          "At least one seating option is required.";
        return false;
      } else {
        this.formErrors.seatingOptionsGeneral = "";
      }
      ...
      const allOptionsValid = this.formErrors.seatingOptionsErrors.every(
    ...
    async convertEventDetailsKey(eventControllerData) {
      ...
      const ticketSaleEndDateTime = new Date(
        eventControllerData.eventDetails.ticketSaleEndDateTime
      );
      const ticketSaleEndDate = ticketSaleEndDateTime
        .toISOString()
        .split("T")[0];
      const ticketSaleEndTime = ticketSaleEndDateTime
        .toTimeString()
        .split(" ")[0];

      const ticketSaleStartDateTime = new Date(
        eventControllerData.eventDetails.ticketSaleStartDateTime
      );
      const ticketSaleStartDate = ticketSaleStartDateTime
        .toISOString()
        .split("T")[0];
      const ticketSaleStartTime = ticketSaleStartDateTime
        .toTimeString()
        .split(" ")[0];
