export default {
    ...
    methods: {
        ...
        submitForm() {
            ...
            if (this.validateForm()) {
                if (this.mode === 'create') {
                    ...
                    console.log(this.formData)
                    const eventId = window.location.pathname.split('/').pop();
                    axios.post('/api/manager/submitEditEvent/' + eventId, this.formData, {
                        headers: {
                            'Content-Type': 'application/json'
                        }
                    })
        ...
        async convertEventDetailsKey(eventControllerData) {
            ...
            const ticketSaleEndDateTime = new Date(eventControllerData.eventDetails.ticketSaleEndDateTime);
            const ticketSaleEndDate = ticketSaleEndDateTime.toISOString().split('T')[0];
            const ticketSaleEndTime = ticketSaleEndDateTime.toTimeString().split(' ')[0];

            const ticketSaleStartDateTime = new Date(eventControllerData.eventDetails.ticketSaleStartDateTime);
            const ticketSaleStartDate = ticketSaleStartDateTime.toISOString().split('T')[0];
            const ticketSaleStartTime = ticketSaleStartDateTime.toTimeString().split(' ')[0];
